...
use Time::Local::POSIX
    qw( timegm timelocal timegm_nocheck timelocal_nocheck );
...
sub _test_group {
    ...
    for my $vals ( @{$group} ) {
        ...
        for my $sub (qw( timelocal timelocal_nocheck )) {
            subtest(
                $sub,
                sub {
                    my $time = __PACKAGE__->can($sub)
                        ->( $sec, $min, $hour, $mday, $mon, $year - 1900 );
        ...
        for my $sub (qw( timegm timegm_nocheck )) {
            subtest(
                $sub,
                sub {
                    my $time = __PACKAGE__->can($sub)
        ...
        for my $key ( sort keys %bad ) {
            subtest(
                ...
                sub {
                    ...
                    local $@ = undef;
                    eval { timegm( $sec, $min, $hour, $mday, $mon, $year - 1900 ) };
...
subtest(
    ...
    sub {
        ...
        local $@ = undef;
        eval { timegm( 0, 0, 0, 29, 1, 100 ) };
        is(
            $@, q{},
            'no error with leap day of 2000 (year passed as 100)'
        );
        ...
        local $@ = undef;
        eval { timegm( 0, 0, 0, 29, 1, 104 ) };
        is(
            $@, q{},
            'no error with leap day of 2004 (year passed as 104)'
        );
        ...
        subtest(
            ...
            sub {
                ...
                is(
                    ...
                    $pre_break,
                    "year $pre_break is treated as current centrury",
                );
                is(
                    (
                    ...
                    $break,
                    "year $break is treated as current century",
                );
                is(
                    (
                        (
